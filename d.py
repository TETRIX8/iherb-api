...
def save_to_csv_batch(items_data_batch, filename="iherb_data.csv"):
    if not items_data_batch:
        logger.warning("Нет данных для сохранения в пакете")
        return

    df = pd.DataFrame(items_data_batch)
    logger.info(f"Сохранение {len(items_data_batch)} записей в CSV файл: {filename}")
...
def main():
    ...
    try:
        ...
        for batch_num, items_json_batch in enumerate(get_items_json_threaded_batched(links_to_process, batch_size=1000)):
            ...
            save_to_csv_batch(items_data_batch)
